...
static const size_t MAX_CALLBACK_COUNT = 64;
	...
	virtual void operator-=(callback_t callback) override final {
		for (size_t i = 0; i < m_callbackCount; i++) {
			if (m_callbacks[i] == callback) {
				memmove(&m_callbacks[i], &m_callbacks[i + 1],
					(m_callbackCount - i - 1) * sizeof(callback_t));
				m_callbackCount--;
				return;
			}
		}
...
public:
	...
	size_t GetMajorVersion() override;
	size_t GetMinorVersion() override;
