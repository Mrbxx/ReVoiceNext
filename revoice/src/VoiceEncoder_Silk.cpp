...
bool VoiceEncoder_Silk::Init(int quality)
{
	...
	if (SKP_Silk_SDK_InitEncoder(m_pEncoder, &this->m_encControl) != SKP_SILK_NO_ERROR) {
		free(m_pEncoder);
		m_pEncoder = nullptr;
		return false;
	}
	...
	if (SKP_Silk_SDK_InitDecoder(m_pDecoder) != SKP_SILK_NO_ERROR) {
		free(m_pDecoder);
		m_pDecoder = nullptr;
		free(m_pEncoder);
		m_pEncoder = nullptr;
