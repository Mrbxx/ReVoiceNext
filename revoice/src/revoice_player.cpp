...
CRevoicePlayer *GetPlayerByIndex(size_t clientId)
{
	if (clientId < 1 || clientId > (size_t)g_RehldsSvs->GetMaxClients()) {
		util_syserror("Invalid player index id=%zu\n", clientId);
	}
...
{
	int clientId = cl->GetId();

	if (clientId < 0 || clientId >= g_RehldsSvs->GetMaxClients()) {
		util_syserror("Invalid player client id=%d\n", clientId);
	}
